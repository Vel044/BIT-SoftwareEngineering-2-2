...
def create_neural_network(input_dim, neurons):
    model = Sequential([
...
for neurons in neurons_list:
    model = create_neural_network(X_train.shape[1], neurons)
    ...
    # 绘制ROC曲线
    plt.plot(fpr, tpr, lw=2, label=f'Neurons: {neurons} (AUC = {roc_auc:.2f})')
...
plt.title('ROC Curve for Different Neuron Counts')
