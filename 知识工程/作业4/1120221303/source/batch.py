...
def create_neural_network(input_dim, dropout_rate=0.1):
    model = Sequential([
...
for batch_size in batch_sizes:
    model = create_neural_network(X_train.shape[1], dropout_rate=0.3)
    ...
    # 绘制ROC曲线
    plt.plot(fpr, tpr, lw=2, label=f'Batch size: {batch_size} (AUC = {roc_auc:.2f})')
...
plt.title('ROC Curve for Different Batch Sizes')
