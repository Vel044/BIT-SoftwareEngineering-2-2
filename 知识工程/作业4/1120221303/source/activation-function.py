...
def create_neural_network(input_dim, activation):
    model = Sequential([
        Dense(16, activation=activation, input_dim=input_dim),
        Dropout(0.3),
...
for activation in activations:
    model = create_neural_network(X_train.shape[1], activation)
    ...
    # 绘制ROC曲线
    plt.plot(fpr, tpr, lw=2, label=f'Activation: {activation} (AUC = {roc_auc:.2f})')
...
plt.title('ROC Curve for Different Activation Functions')
