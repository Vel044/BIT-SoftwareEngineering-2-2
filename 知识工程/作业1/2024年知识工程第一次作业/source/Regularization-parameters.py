...
def train_glm(features, labels):
    ...
    model = LogisticRegression(
        solver='saga', class_weight='balanced', max_iter=10, tol=1e-4, verbose=1)
...
def compare_regularization_parameters(features, labels, regularization_values):
    X_train, X_test, y_train, y_test = train_test_split(
        features, labels, test_size=0.2, random_state=42)
    ...
    for C in regularization_values:
        model = LogisticRegression(
            solver='saga', class_weight='balanced', C=C, max_iter=100, tol=1e-4)
        model.fit(X_train, y_train)
    ...
    plt.title('ROC Curve Comparison for Different Regularization Parameters')
...
# 使用已有的特征和标签数据
compare_regularization_parameters(features, labels, [0.01, 0.1, 1, 10, 100])
