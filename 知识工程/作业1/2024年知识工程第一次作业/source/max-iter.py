...
def compare_max_iter(features, labels, iterations):
    X_train, X_test, y_train, y_test = train_test_split(
        features, labels, test_size=0.2, random_state=42)
    ...
    for max_iter in iterations:
        model = LogisticRegression(
            solver='saga', class_weight='balanced', max_iter=max_iter, tol=1e-4)
        model.fit(X_train, y_train)
        y_pred_prob = model.predict_proba(X_test)[:, 1]
        fpr, tpr, _ = roc_curve(y_test, y_pred_prob)
        auc_score = roc_auc_score(y_test, y_pred_prob)
        plt.plot(
            fpr, tpr, label=f'max_iter = {max_iter} (AUC = {auc_score:.2f})')

    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve Comparison for Different max_iter Values')
...
# 使用已有的特征和标签数据
compare_max_iter(features, labels, [10, 50, 100, 200])
