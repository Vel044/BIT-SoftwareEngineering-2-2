...
def train_and_plot_roc(features, labels, class_weights):
    X_train, X_test, y_train, y_test = train_test_split(
        features, labels, test_size=0.2, random_state=42)
    ...
    for weight in class_weights:
        model = LogisticRegression(
            solver='saga', class_weight=weight, max_iter=100, tol=1e-4)
        model.fit(X_train, y_train)
        y_pred_prob = model.predict_proba(X_test)[:, 1]
        fpr, tpr, _ = roc_curve(y_test, y_pred_prob)
        auc_score = roc_auc_score(y_test, y_pred_prob)
        label = f'Class Weight = {weight}' if weight else 'Class Weight = None'
        plt.plot(fpr, tpr, label=f'{label} (AUC = {auc_score:.2f})')

    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve Comparison for Different Class Weights')
...
# 使用已有的特征和标签数据
train_and_plot_roc(features, labels, [None, 'balanced'])
